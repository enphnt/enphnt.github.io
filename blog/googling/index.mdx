...
hero_image: "./hero.jpg"
hero_image_alt: "me searching for some dumb stuff"
hero_image_credit_text: "Pixabay"
hero_image_credit_link: "https://www.pexels.com/photo/coffee-apple-iphone-smartphone-38547/"
tags: ["google", "web dev", "productivity", "search"]
...
<div className={note}>
  <strong>Fun Fact:</strong> In its infancy, Google search was originally called BackRub. This is thought to be an ode to the [backlink](https://en.wikipedia.org/wiki/Backlink) retrieval concept of the system. So next time you search, enjoy the free massage.
</div>

# You know a few tricks, but did you know...
...
Search `"Frank Foster"` to get results that contain both the first and last name of this amazing jazz saxophonist.

This can be extended to other terms too. In this case, it appears a country Western artist shares the same name. So to find the sax legend, ensure that all search results also contain the term 'jazz':
...
```
<sup>Pop this text into the Google search bar and submit</sup>
...
Get results that are a specified file type.
...
For example, what if you have 2 favorite coffees and you want to look at the results for both:
...
Looking for a new metronome that's within your budget? Try:
...
For example, if you want your Nick Cage images for the desktop try something like:
...
<br />
## 8. Use asterisk `*` for a wildcard search
Wherever you place the * character it will serve as a wildcard and match any phrase or word. It's more useful to review search autocompletion results when you want to find some missing part of your search.

For example, you could use it to see what things you could do with javascript:

    ![google-asterisk](./google-asterisk.png)


# Lvl 3 - Senior Dev

<br />
## 9. Check the latest Googlebot indexes using `cache:`
...
This will redirect you to the most recent cache of the HTML page. Importantly, Javascript is not included in the cache as it's just a cache of plain HTML only.
...
For example, let's inspect `guitarcenter.com`:
    ...
    ![google-guitarcenter](./google-guitarcenter.png)
...
Clicking any of the resulting links reveals that the page is non-existent. So ultimately, there appears to be a Google search indexing issue with these old listings.

<br />
## 11. Find those who steal from you!
So you have a blog and you just crafted some amazing informative posts but you're afraid others may copy your work. Well, aside from feeling flattered, if you want to identify websites that are repurposing your content, you can!
...
Notice that I've ignored my own site and have just targeted a portion of the page title. This is because not many results exist for this title portion.

I reviewed the results and it appears no one is copying me...yet! I notice that my pages here are not being properly indexed. As a result, in a future blog version, I'll be configuring SEO using Gatsby's official support strategy. More on that to come.

# Conclusion
These tools are all reasonably reliable in my testing. Without them, this info would largely remain hidden and inaccessible. In the future, I see these operators, or at least their principles, becoming more important. ChatGPT has highlighted that we will have to carefully craft our questions in requesting information from AI resources else we will get clumsy answers in return. As the saying goes, 'garbage in, garbage out'.

I hope some of these are useful for hunting the next thing you need to know and reducing some of the garbage that Google Search throws at us.

![thanks-google](./google-oscar.jpeg)
