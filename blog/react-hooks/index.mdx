...
tags: ["react", "web dev", "react hooks", "dev interview"]
...
The motiation is that class components become unweildy and its hard to share logic between them. So hooks comes in as a much more direct API to get to all the React goodness without having to grasp classes or even `this`. (*〠_〠;)
...
<table>
  <tbody>
    ...
    <tr>
      <td className={tableTextAlignTop}>useState</td>
      <td>
        ```javascript 
        ...
        ```
        <>This hooks is the one I know most. Use it to handle 'reactive' data. When data changes state, component will rerender.</>
      </td>
    </tr>
    <tr>
      <td className={tableTextAlignTop}>useEffect</td>
      <td>
    ...
    <tr>
      <td className={tableTextAlignTop}>useContext</td>
      <td>
        Want to pass data and avoid props? The example from React docs shows how we can pass the `value` from the `ThemeContextProvider\>` so that its used by the child components.
    ...
    <tr>
      <td className={tableTextAlignTop}>useReducer</td>
      <td>
        An alternative way to set state using a reducer `(state, action) => newState`. It returns the current state paired with a `dispatch` method. It's necessary when new state depends on old state.
        <br />
        <br />
    ...
    <tr>
      <td className={tableTextAlignTop}>useMemo</td>
      <td>
    ...
    <tr>
      <td className={tableTextAlignTop}>useCallback</td>
      <td>
        Similar to `useMemo()`, useful for reducing performance costs of expensive functions.
    ```javascript
    ...
    <tr>
      <td className={tableTextAlignTop}>useRef</td>
      <td>
